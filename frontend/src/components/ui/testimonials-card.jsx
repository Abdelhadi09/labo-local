    ...
    return (
        <div className={cn("flex items-center justify-center p-8", className)}>
            ...
            >
                {/* Counter */}
                {showCounter && (
                    <div className="row-start-1 md:col-start-2 md:row-start-1 text-center font-mono text-sm text-neutral-500 light:text-neutral-400">
                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className="absolute inset-0 w-full h-full overflow-hidden border-[6px] bg-neutral-200 light:bg-neutral-800 border-white light:border-neutral-700 shadow-2xl rounded-lg"
                                    initial={{
                                        x: offset * 15,
                                        y: Math.abs(offset) * 6,
                ...
                {showNavigation && items.length > 1 && (
                    <div className="col-start-1 md:col-start-2 md:row-start-3 flex gap-2 m-auto -mt-2 md:mt-4 md:m-0">
                        <button
                            ...
                            onClick={handlePrev}
                            className={cn(
                                "flex items-center justify-center w-10 h-10 rounded-full border border-neutral-200 light:border-neutral-700 bg-white light:bg-neutral-800 transition-all",
                                activeIndex === 0
                                    ? "opacity-50 cursor-not-allowed"
                                    : "hover:bg-neutral-100 light:hover:bg-neutral-700 hover:scale-105"
                            )}
                        ...
                        >
                            <ArrowRight className="w-4 h-4 text-neutral-700 light:text-neutral-300" />
                        </button>
                    </div>
                )}
            </div>
